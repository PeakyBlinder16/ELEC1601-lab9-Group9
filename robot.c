...
int checkRobotSensorLeftAllWalls(struct Robot * robot, struct Wall_collection * head) {
    ...
    {
        ptr = head_store;
        xDir = round(robotCentreX+(-ROBOT_WIDTH/2+10)*cos((robot->angle)*PI/180-90)-(-ROBOT_HEIGHT/2-10-SENSOR_VISION+sensorSensitivityLength*i)*sin((robot->angle-90)*PI/180));
        yDir = round(robotCentreY+(-ROBOT_WIDTH/2+10)*sin((robot->angle)*PI/180-90)+(-ROBOT_HEIGHT/2-10-SENSOR_VISION+sensorSensitivityLength*i)*cos((robot->angle-90)*PI/180));
        xTL = (int) xDir;
        yTL = (int) yDir;
        hit = 0;
...
int checkRobotSensorFrontAllWalls(struct Robot * robot, struct Wall_collection * head) {
    ...
    {
        ptr = head_store;
        xDir = round(robotCentreX+(-ROBOT_WIDTH/2+10)*cos((robot->angle)*PI/180)-(-ROBOT_HEIGHT/2-10-SENSOR_VISION+sensorSensitivityLength*i)*sin((robot->angle)*PI/180));
        yDir = round(robotCentreY+(-ROBOT_WIDTH/2+10)*sin((robot->angle)*PI/180)+(-ROBOT_HEIGHT/2-10-SENSOR_VISION+sensorSensitivityLength*i)*cos((robot->angle)*PI/180));
        xTL = (int) xDir;
        yTL = (int) yDir;
        hit = 0;
...
void robotUpdate(struct SDL_Renderer * renderer, struct Robot * robot){
    ...
    SDL_SetRenderDrawColor(renderer, 28, 28, 28, 28);
    ...
    */
    ...
    for (i = 0; i < 7; i++)
    {
        xDir = round(robotCentreX+(-ROBOT_WIDTH/2+10)*cos((robot->angle)*PI/180-90)-(-ROBOT_HEIGHT/2-10-SENSOR_VISION+sensor_sensitivity*i)*sin((robot->angle-90)*PI/180));
        yDir = round(robotCentreY+(-ROBOT_WIDTH/2+10)*sin((robot->angle)*PI/180-90)+(-ROBOT_HEIGHT/2-10-SENSOR_VISION+sensor_sensitivity*i)*cos((robot->angle-90)*PI/180));
        xTL = (int) xDir;
        yTL = (int) yDir;

        SDL_Rect rect = {xTL, yTL, 2, sensor_sensitivity};
        SDL_SetRenderDrawColor(renderer, 150+(20*(5-i)), 50+(20*(5-i)), 200+(20*(5-i)), 255);
        SDL_RenderDrawRect(renderer, &rect);
        SDL_RenderFillRect(renderer, &rect);
    }
    ...
    for (i = 0; i < 7; i++)
    {
        xDir = round(robotCentreX+(-ROBOT_WIDTH/2+10)*cos((robot->angle)*PI/180)-(-ROBOT_HEIGHT/2-10-SENSOR_VISION+sensor_sensitivity*i)*sin((robot->angle)*PI/180));
        yDir = round(robotCentreY+(-ROBOT_WIDTH/2+10)*sin((robot->angle)*PI/180)+(-ROBOT_HEIGHT/2-10-SENSOR_VISION+sensor_sensitivity*i)*cos((robot->angle)*PI/180));
        xTL = (int) xDir;
        yTL = (int) yDir;
...
void robotAutoMotorMove(struct Robot * robot, int left_sensor, int front_sensor, int right_sensor) {
    ...
    else if ( front_sensor != 0 && right_sensor == 0) {
        if(turn > 0){
            turn = 0;
        }
        turn --;
    ...
    else if (front_sensor == 0 && (left_sensor != 0 || right_sensor != 0)) {
        robot->direction = UP;
    }
    else if (front_sensor != 0 && (left_sensor != 0 && right_sensor != 0)) {
        robot->direction = DOWN;
